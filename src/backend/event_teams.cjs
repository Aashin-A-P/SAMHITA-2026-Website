...
module.exports = function (db) {
  ...
  router.post('/', async (req, res) => {
    const { eventId, teamName, members, createdBy } = req.body;

    if (!eventId || !teamName || !Array.isArray(members) || !createdBy) {
    ...
    try {
      ...
      const [result] = await db.execute(
        `INSERT INTO event_teams (eventId, teamName, createdBy, member1Id, member2Id, member3Id, member4Id)
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [
          eventId,
          teamName,
