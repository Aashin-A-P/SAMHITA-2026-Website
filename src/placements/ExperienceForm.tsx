...
const ExperienceForm: React.FC<ExperienceFormProps> = ({ onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-gray-900/70 backdrop-blur-md border border-gold-500/30 p-8 rounded-lg w-full max-w-lg">
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="mb-4">
              ...
              <input
                ...
                className="w-full bg-gray-800/80 border border-gold-500/30 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-gold-500"
                required
              />
            </div>
            <div className="mb-4">
              <label htmlFor="type" className="block text-gold-400 mb-2">Type</label>
              <select
                id="type"
                value={type}
            ...
            <div className="mb-4">
              <label htmlFor="year" className="block text-gold-400 mb-2">Year of Passing Out</label>
              <input
                type="text"
                id="year"
                value={year}
            ...
            <div className="mb-4">
              <label htmlFor="company" className="block text-gold-400 mb-2">Company</label>
              <input
                type="text"
                id="company"
                value={company}
          ...
          <div className="mb-6">
            <label htmlFor="pdf" className="block text-gold-400 mb-2">Upload PDF</label>
            <input
              type="file"
              id="pdf"
              accept=".pdf"
              onChange={(e) => setPdf(e.target.files ? e.target.files[0] : null)}
              className="w-full text-white file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-samhita-600 file:text-white hover:file:bg-samhita-700"
            />
          </div>
          <div className="flex justify-end gap-4">
            ...
            <button
              type="submit"
              className="px-6 py-2 bg-samhita-600 text-white font-semibold rounded-lg hover:bg-samhita-700 transition-colors glow-button"
            >
              Submit
            </button>
          </div>
